...
public class Nivel_1Activity extends AppCompatActivity {

    private FirebaseFirestore db;
    private FirebaseAuth auth;
    private String userId;

    private String nivelActual = "abecedario";
    ...
    private void mostrarFrase() {
        ...
        for (char letra : respuestaCorrecta.toLowerCase().toCharArray()) {
            int imgId = getResources().getIdentifier("letra_" + letra, "drawable", getPackageName());
            if (imgId != 0) {
    ...
    private void verificarNivelCompletado() {
        ...
        if (completado) {
            ...
            fraseIndex = 0;
            List<HashMap<String, Object>> nuevasFrases = new ArrayList<>();
            for (HashMap<String, Object> frase : frases) {
                if (!Boolean.TRUE.equals(frase.get("respondido"))) {
                    nuevasFrases.add(frase);
                }
            }

            if (!nuevasFrases.isEmpty()) {
                frases.clear();
                frases.addAll(nuevasFrases);
                mostrarFrase();
            } else {
                Toast.makeText(this, "Error al reiniciar frases incorrectas.", Toast.LENGTH_SHORT).show();
                cargarFrasesDelNivel();
            }
        }
    }
